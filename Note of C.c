...
/*2*/
	对于以i作为index variable且为分母（如求平均数）的情况，注意讨论i=0的情况，i=0时认为平均值为0
...
/*18*/
	scanf中double要用%lf读
	每次调用用sscanf读同一个string时，都是从头开始。
	而用fgets之类读文件时都是顺序都下去。
	rewind() 使文件内部的位置指针重新指向开头

/*19*/
	%-2d 如果没有2，那么-没有用，所谓左右对齐是基于占位进行的
	对于 float 类型的变量，printf()中的说明符可以用%f或%lf，而scanf()中的说明符则只能用%f；
	对于 double 类型的变量，printf() 中的说明符可以用%f或%lf，而scanf()中的说明符则只能用%lf；
	对于 long double 类型的变量，printf()中的说明符可以用%f或%lf，而scanf()中的说明符则只能用%lf。

/*20*/
	对于 例如
		unsigned short int k = 700000;
	由于 unsigned short int 的范围是0~65535，所以会溢出输出。
	但是如果是
...
/*21*/
	scanf不会输入space character， 而gets会，二者都以此为结束标志

/*22*/
	scanf 返回读入的数据的项数，都到末尾返回EOF。

/*23*/
	优先级
	相同优先级，一般从左至右运算。除了[一元运算符][条件运算符][赋值运算符]之外。
	指针优先级最大，然后是一元运算符，再是多元运算符；
	最后逻辑运算符

/*24*/
	如果没有超过8的数字\后面的数字默认是8进制的，对应的ASCII就是输出的字符
	如'\102' 是指八进制数代表十进制的66D，即B。
...
/*27*/
	转义字符后面可以跟8进制数来表示一个字符，其形式为：\nnn，n的个数最多三位，最小一位，按最大匹配进行解释。与8进制常数不同的是，开头的0也算一位
...
/*31*/
	...
	事实上，类型本身只有在解析的时候才会真正存在，在内存中，unsigned char 和 signed char 都是一串 bit 而已，所以对 unsigned char 的变量赋值 -1 的时候，会当作255.
...
/*34*/
	进行调试找bug时，可以用printf变量的方法进行跟踪；

/*35*/
	定义record时，内部的指针本身不用分配空间（只要为结构类型的指针分配了空间），但它指向的对象依旧要分配。在使用结构时最容易出错的是非法内存访问。
	教材库里的ReadLine 似乎不用再分配空间
...
/*37*/
	...
	char *temp;
	if (temp == 'a') Process();
	这样的话编译器不会报错，可能有warming...竟然无法识别 char *和 char
